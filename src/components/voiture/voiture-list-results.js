...
export const VoitureListResults = ({ voitures, ...rest }) => {
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedCustomerIds = voitures.map((customer) => customer.id);
    } else {
      ...
      <PerfectScrollbar>
        <Box sx={{ minWidth: 1050 }}>
          <Table>
            ...
            <TableBody>
              {voitures.slice(0, limit).map((customer) => (
                <TableRow
                  hover
                  key={customer.id}
...
VoitureListResults.propTypes = {
  voitures: PropTypes.array.isRequired,
};
